...
const Edit = () => {
    ...
    useEffect(() => {
        const ListOfProductsURL = `http://localhost:5173/product/GetProductById?id=${id}`;
...
+            reader.readAsDataURL(e.target.files[0]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const UploadImageURL = `http://localhost:5125/product/uploadfile?id=${data.id}`;
            const UpdateProductURL = "http://localhost:5125/product/updateproduct";
            if(data.image.name){
