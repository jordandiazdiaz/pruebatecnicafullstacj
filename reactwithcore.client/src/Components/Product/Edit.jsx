...
const Edit = () => {
    ...
    useEffect(() => {
        const ListOfProductsURL = `${config.getProductById}?id=${data.id}`; 
...
+            reader.readAsDataURL(e.target.files[0]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const UploadImageURL = `${config.uploadfile}?id=${data.id}`;
            const UpdateProductURL = config.REACT_APP_UPDATE_PRODUCT;
            if(data.image.name){
